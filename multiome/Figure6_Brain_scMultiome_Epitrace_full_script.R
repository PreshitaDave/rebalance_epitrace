...
splicedm <- readRDS('./Packed_Figure_Reproducibility/input_data_for_EpiTrace/Figure6/matrix/Brain_scMultiome_spliced_mtx_for_CreateAssay.rds')
...
# Add meta 
readRDS('./Packed_Figure_Reproducibility/input_data_for_EpiTrace/Figure6/meta/Figure6_Brain_scMultiome_meta.rds') -> cell_meta 

epitrace_obj_age_conv_estimated@meta.data -> temp 
left_join(temp, cell_meta ) -> temp22
...
epitrace_obj_age_conv_estimated[['rna_spliced']] <- Seurat::CreateAssayObject(counts = splicedm[,epitrace_obj_age_conv_estimated$cell],min.cells = 0,min.features = 0,check.matrix = F)
...
Signac::CreateChromatinAssay(counts = GetAssayData(epitrace_obj_age_estimated_multiome,slot = 'data'),min.cells = 0,min.features = 0,genome = BSgenome.Hsapiens.UCSC.hg38@seqinfo) -> epitrace_obj_age_estimated_multiome[['all_chrom_assay']]
...
open.peaks <- AccessiblePeaks(epitrace_obj_age_estimated_multiome, idents = c("3", "0"))

# match the overall GC content in the peak set
meta.feature <- GetAssayData(epitrace_obj_age_estimated_multiome, assay = "all_chrom_assay", slot = "meta.features")
...
subset(epitrace_obj_age_estimated_multiome,seurat_clusters %in% c(3,0,11,9,5,8)) -> tt1
tt1$class <- ifelse(tt1$seurat_clusters %in% c(0,11,5,9),'NR2F1+','LMO3+')
...
pl2 <- lapply(pl1,function(x){
  ...
  ggplot(xdata,aes(x=UMAP_1,y=UMAP_2,fill=value))+geom_point(size=2.2,pch=21,color='darkgray')+xlim(c(-10.2,4.5)) + ylim(c(-9,4)) + scale_fill_gradientn(colors=c('cornflowerblue','beige','red')) + theme_void() + ggtitle(feature_new_id) + theme(text=element_text(size=10))
})
...
pl4 <- lapply(pl3,function(x){
  ...
  x$labels$title -> featureid
  xdata <- arrange(xdata,value)
  ggplot(xdata,aes(x=UMAP_1,y=UMAP_2,fill=value))+geom_point(size=2.2,pch=21,color='darkgray')+xlim(c(-10.2,4.5)) + ylim(c(-9,4)) + scale_fill_gradientn(colors=c('cornflowerblue','beige','red')) + theme_void() + ggtitle(featureid) + theme(text=element_text(size=20))
})
...
subset(epitrace_obj_age_estimated_multiome,seurat_clusters %in% c(3,0,11,9,5,8)) -> tt1
tt1$class <- ifelse(tt1$seurat_clusters %in% c(0,11,5,9),'NR2F1+','LMO3+')
tt1@meta.data[tt1$seurat_clusters %in% c(0,3),] -> nIPC_cytotrace_Epitrace_df
